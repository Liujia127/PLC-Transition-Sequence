        ...
        for start in range(n - window_size + 1):
            ...
            for i in range(start + 1, n - window_size + 1):
                if window == text[i:i + window_size]:
                    repeats.append(window)

            if len(repeats) > 0:
                if window_size > max_length:
                    max_length = window_size
                    longest_repeats = repeats
                elif window_size == max_length:
                    longest_repeats.extend(repeats)
    ...
    for parent, children in substring_groups.items():
        ...
        if len(candidate_sets) > 1:
            ...
            for string in candidate_sets:
                ...
                if len(cur) > 0:
                    for string in cur:
                        if string not in candidate_sets:
                            next.append(string)
                    # else:
                    #     next.append(str)
            print("abv")
        ...
        i = i + 1
        for string in candidate_sets:
            if string not in candidate_sum:
...
string1 = ""
result = find_sorted_substrings(string1)
...
cand = candidate_sets(substring_groups)
print(f"Cand:{cand}")
# print(result)
...
for string in cand:
    for substring, freq in result:
        if string == substring or string in substring:
            cand_fre.append((string, freq))
            break

cand_fre.sort(key=lambda x: x[1], reverse=True)  # 按频率降序排序
print(cand_fre)
